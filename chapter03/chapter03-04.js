...
function Image (data, x, y, name) {
        if (x<=0 || y<=0) throw "Values must be higher";
        if(data.length != x*y) throw "Data size doesn't correspond with width or height";
        this.width = x;          
        this.height = y;
        this.name = name; 
        this.data=data;
...
Image.prototype.pixelData = function (x, y){        // x= column and y=row 
        if(x<=0 || y<=0) throw "Values must be higher";
        if(x>this.width || y>this.height) throw "The pixel must correspond to the image size";
